#!/bin/sh
#
# chmiel.sh - beer™ develpment environment installation script based on (LARBS)[https://larbs.xyz]
# License: GNU GPLv3
...
[ -z "$dotfilesrepo" ] && dotfilesrepo="https://github.com/jaenek/dotfiles.git"
[ -z "$progsfile" ] && progsfile="https://raw.githubusercontent.com/jaenek/chmiel/master/progs.csv"
...
welcomemsg() { \
	dialog --title "Welcome!" --msgbox "This script will set up beer™ development environment!" 10 60
...
usercheck() { \
	...
	dialog --colors --title "WARNING!" --yes-label "CONTINUE" --no-label "No wait..." --yesno "The user \`$name\` already exists on this system. Installation will \\Zboverwrite\\Zn any conflicting settings/dotfiles on the user account. Your user files, documents, videos, etc. will \\Zbnot\\Zn be overwriten, so don't worry about that, but only click <CONTINUE> if you don't mind your settings being overwritten.\\n\\nNote also that this will change $name's password to the one you just gave." 14 70
	}
...
newperms() { # Set special sudoers settings for install (or after).
	sed -i "/#beer™/d" /etc/sudoers
	echo "$* #beer™" >> /etc/sudoers ;}
...
maininstall() { # Installs all needed programs from main repo.
	dialog --title "beer™ Installation" --infobox "Installing \`$1\` ($n of $total). $1 $2" 5 70
	installpkg "$1"
	}
...
aurinstall() { \
	dialog --title "beer™ Installation" --infobox "Installing \`$1\` ($n of $total) from the AUR. $1 $2" 5 70
	echo "$aurinstalled" | grep -q "^$1$" && return 1
	sudo -u "$name" $aurhelper -S --noconfirm "$1" >/dev/null 2>&1
	}

pipinstall() { \
	dialog --title "beer™ Installation" --infobox "Installing the Python package \`$1\` ($n of $total). $1 $2" 5 70
...
for x in curl ca-certificates base-devel git ntp zsh ; do
	dialog --title "beer™ Installation" --infobox "Installing \`$x\` which is required to install and configure other programs." 5 70
	installpkg "$x"
done
...
newperms "%wheel ALL=(ALL) NOPASSWD: ALL # beer™"
